...
class DemandeAdhesion(TimestampUseModel):
    ...
    def __str__(self):
        return f"{self.prenoms} {self.nom} {self.courriel}"
...
class Membre(models.Model):
    ...
    LISTE_choix = [
        ("PR", "Président"),
        ("VP", "Vice-Président"),
        ("SG", "Sécrétaire Générale"),
        ("TR", "Trésorière Générale"),
        ("VT", "Vice-Trésorière"),
        ("C1", "Premier Commissaire aux comptes"),
        ("C2", "Second Commissaire aux comptes"),
        ("Mb", "Membre actif"),
    ]

    demande = models.OneToOneField(
        DemandeAdhesion, on_delete=models.CASCADE, primary_key=True
    )
